...
const ROOT_AUTH_CONFIG_FILE = path.join(process.cwd(), 'auth_config.json');
const SESSIONS_FILE = path.join(DATA_DIR, 'sessions.json');
const ROOT_SESSIONS_FILE = path.join(process.cwd(), 'sessions.json');

const SESSION_TTL_MS = 365 * 24 * 60 * 60 * 1000; // 365 days lifetime - prevent unexpected admin logout
...
function loadSessions() {
  try {
    ...
    } else if (fs.existsSync(ROOT_SESSIONS_FILE)) {
      raw = fs.readFileSync(ROOT_SESSIONS_FILE, 'utf8');
    }
    if (raw) {
...
function saveSessions(sessionsMap) {
  try {
    ...
    fs.writeFileSync(SESSIONS_FILE, str, 'utf8');
    try { fs.writeFileSync(ROOT_SESSIONS_FILE, str, 'utf8'); } catch (e) {}
  } catch (err) {
